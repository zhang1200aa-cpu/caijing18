...
def get_latest_ai_summary(range_key: str = '1d') -> dict:
    ...
    try:
        ...
        print(f"❌ [AI 总结] 查询失败: {e}")
        return None
    finally:
        session.close()

def get_ai_summary_by_date(range_key: str, date_label: str) -> dict:
    """按日期获取指定范围的 AI 总结"""
    session = get_session()
    try:
        summary = session.query(AISummary).filter(
        ...
        print(f"❌ [AI 总结] 按日期查询失败: {e}")
        return None
    finally:
        session.close()

def get_ai_summaries_by_date_range(range_key: str, start_label: str, end_label: str) -> list:
    """
    按日期范围获取 AI 总结列表
    
    Args:
        range_key: 范围键值 '1d'/'3d'/'1w'
        start_label: 起始日期 YYYY-MM-DD（包含）
        end_label: 结束日期 YYYY-MM-DD（包含）
    ...
    try:
        summaries = session.query(AISummary).filter(
            AISummary.range_key == range_key,
            AISummary.date_label >= start_label,
            AISummary.date_label <= end_label
